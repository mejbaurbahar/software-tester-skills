...
## Coverage Theater - High Numbers, Low Value
...
### **Detection Pattern**
expect\(.*\)\.toBeTruthy|assert.*!= null|coverage.*\d{2}%

## Test-Induced Damage - Code Warped for Testability
...
### **Detection Pattern**
interface I\w+Provider|Factory|TestDouble|MockBuilder

## Flaky Test Tolerance - The Boy Who Cried Wolf
...
### **Detection Pattern**
flaky|retry|intermittent|sleep\(|setTimeout\(.*\d{4}

## Test Pyramid Inversion - E2E Heavy, Unit Light
...
### **Detection Pattern**
describe.*E2E|playwright|cypress|selenium|puppeteer

## Mock Contamination - Tests That Test Mocks
...
### **Detection Pattern**
mock\(|jest\.mock|stub\(|when\(.*\)\.thenReturn

## Test Data Time Bomb - Hardcoded Dates
...
### **Detection Pattern**
new Date\(['"]\d{4}-|Date\.parse|moment\(['"]\d

## Shared State Pollution - Tests That Affect Each Other
...
### **Detection Pattern**
beforeAll.*insert|global\.|singleton|static.*=

## Missing Edge Cases - Happy Path Tunnel Vision
...
### **Detection Pattern**
\{ name:.*valid|happy.*path|success.*scenario

## Assertion-Free Test - The Smoke Screen
...
### **Detection Pattern**
expect\(.*\)\.toBeTruthy|expect\(.*\)\.toBeDefined|expect\(.*\)\.not\.toBeNull